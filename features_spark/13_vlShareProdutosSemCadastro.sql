...
--   cadastro AUSENTE (NULL). Feature de **missingness** — captura ausência
--   que a média (AVG) NÃO vê (descarta NULL; `product_photos_qty` não tem 0,
--   só NULL). Sinal de qualidade/engajamento do seller.
-- GRÃO: atributo do PRODUTO -> PRODUTOS DISTINTOS por seller; denominador =
--   nº de produtos distintos do seller.
-- ⚠ No Olist (corte 2018-07-01) os 580 sem categoria == sem descrição == sem
--   foto (colineares neste dataset); `SemPeso` é quase constante (2 produtos).
...
-- ----------------------- Prova C — distribuição do share (catálogo todo sem cadastro) -----------------------
WITH produtos AS (
