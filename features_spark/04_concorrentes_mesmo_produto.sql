...
meus_prod_d28  AS (SELECT DISTINCT seller_id, product_id FROM vendas WHERE dt_venda >= timestamp(:data_corte) - INTERVAL 28 DAYS),
meus_prod_d56  AS (SELECT DISTINCT seller_id, product_id FROM vendas WHERE dt_venda >= timestamp(:data_corte) - INTERVAL 56 DAYS),
meus_prod_d365 AS (SELECT DISTINCT seller_id, product_id FROM vendas WHERE dt_venda >= timestamp(:data_corte) - INTERVAL 365 DAYS),
