...
--   cubagem = product_length_cm × product_height_cm × product_width_cm, de
--   `products` (1 linha por product_id) — IMUTÁVEL. A MÉDIA é ESTÁTICA:
--   calculada UMA vez sobre os PRODUTOS DISTINTOS já vendidos pelo seller
--   (toda a vida < corte), alinhando com descrição/fotos (§3).
...
-- Nulos    : qualquer dimensão NULL -> cubagem NULL (ignorada).
...
-- TOTAL por UNIDADE (volume embarcado), 4 janelas (cresce no tempo).
total AS (
    SELECT seller_id,
...
-- PRODUTOS DISTINTOS por seller (toda a vida; cub constante por produto).
-- Base ESTÁTICA da média — sem janelas, pois a cubagem não muda no tempo.
produtos AS (
