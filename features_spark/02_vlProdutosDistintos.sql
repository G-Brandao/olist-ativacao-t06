...
-- ---------------------------------------------------------------------
-- Colunas  : vlProdutosDistintos{D14,D28,D56,D365,Vida}
...
por_seller AS (
    SELECT seller_id,
        COUNT(DISTINCT CASE WHEN datediff(data_corte, dt_venda) <= 14  THEN product_id END) AS d14,
        COUNT(DISTINCT CASE WHEN datediff(data_corte, dt_venda) <= 28  THEN product_id END) AS d28,
        COUNT(DISTINCT CASE WHEN datediff(data_corte, dt_venda) <= 56  THEN product_id END) AS d56,
        COUNT(DISTINCT CASE WHEN datediff(data_corte, dt_venda) <= 365 THEN product_id END) AS d365,
